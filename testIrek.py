...
while True:
  if btnA.wasPressed():
    ...
    wait(0.5)
  elif btnC.isPressed():                            #color control of each LED:
  ...
  if i%(3+5*10)==0: # x - number of repetitions of the cycles, 0 = standard ; by changing the number of LEDs at a time, the length of the cycle changes 2 LEDs = i%(3+5*x), 1 LED = i%(4+5*x)
    j *= -1
